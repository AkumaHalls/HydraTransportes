...
if (!MAPBOX_TOKEN) {
  console.warn('MAPBOX_TOKEN não configurado - geocodificação não funcionará');
}
...
setInterval(() => {
  const now = Date.now();
  for (const [key, val] of GEOCODE_CACHE.entries()) {
    if (now - val.ts > GEOCODE_CACHE_TTL) GEOCODE_CACHE.delete(key);
  }
}, 60 * 60 * 1000);

async function mapboxGeocode(query) {
  const cacheKey = `mb:${query.toLowerCase().trim()}`;
...
exports.calcular = async (req, res) => {
  try {
    ...
    let origCoords, destCoords;
    if (origemLat && origemLng) {
      origCoords = { lat: parseFloat(origemLat), lng: parseFloat(origemLng) };
    } else {
      origCoords = await geocodeAddress(origem);
    }
    if (destinoLat && destinoLng) {
      destCoords = { lat: parseFloat(destinoLat), lng: parseFloat(destinoLng) };
    } else {
